...
class DateTimeWidget(MultiWidget):
    ...
    def __init__(self, attrs=None, options = {}):
        ...
        self.option = ()
        self.option += (options.get('format','yyyy-mm-dd hh:ii'),)
        ...
        self.option += (options.get('todayBtn','true'),)
        self.option += (options.get('todayHighlight','false'),)
        self.option += (options.get('minuteStep','15'),)
        ...
        self.format = self.option[0]
        self.django_format = pattern.sub(lambda x: dateConversion[x.group()], self.option[0])

        widgets = (DateTimeInput(attrs=attrs, format=self.django_format),)

        super(DateTimeWidget, self).__init__(widgets, attrs)

    def value_from_datadict(self, data, files, name):
        date_time = [widget.value_from_datadict(data, files, name + '_%s' % i) for i, widget in enumerate(self.widgets)]
        dt = u''
        try:
            if date_time[0]:
                dt = to_current_timezone(datetime.strptime(date_time[0], self.django_format))
        except ValueError:
            return u''
        else:
            return str(dt)
    ...
    def decompress(self, value):
        if value:
            if type(value) is str:
                ...
                value = to_current_timezone(parser.parse(value))
            else:
